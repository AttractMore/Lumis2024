...
function be_display_posts_shortcode( $atts ) {
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $category_display && is_object_in_taxonomy( get_post_type(), $category_display ) ) {
			...
			if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
				...
				$category_display_text = ' <span class="category-display"><span class="category-display-label">' .
				                         $category_label . '</span> ' . implode( ' &#124; ', $term_output ) . '</span>';
			}
		...
		$output = '		          <' . $inner_wrapper . ' class="' . implode( ' ', $class ) .
		          '">' . '<div class="post_image">'.$image.'</div>' .'<div 
		          class="post_details">'.$title . $date . $author . $category_display_text . $excerpt . $content.'</div>' . '</'.$inner_wrapper .'>';
