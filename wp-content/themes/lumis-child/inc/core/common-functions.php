...
if ( ! function_exists( 'astra_the_post_title' ) ) {
	...
	function astra_the_post_title_read_more( $before = '', $after = '', $post_id = 0, $echo = true ) {
		...
		if ( $enabled ) {
			...
			// This will work same as `the_title` function but with Custom Title if exits.
			if ( $echo ) {
				echo $before . 'Read More' . $after; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			} else {
				return $before . $title . $after;
			}
		}
