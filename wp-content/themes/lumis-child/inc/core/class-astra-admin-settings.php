...
if (!class_exists("Astra_Admin_Settings")) {
  ...
  class Astra_Admin_Settings
  {
    ...
    {
      /*self::$menu_page_title = apply_filters( 'astra_menu_page_title', __( 'Additional Options', 'astra' ) );
      ...
      if (current_user_can("install_plugins") && !defined("ASTRA_SITES_NAME") && "1" == get_option("fresh_site")) {
        ...
        $astra_sites_notice_args = [
          ...
          "message" => sprintf(
            ...
            __(
              'Did you know Astra comes with dozens of ready-to-use <a href="https://wpastra.com/ready-websites/?utm_source=install-notice">starter templates</a>? Install the Starter Templates plugin to get started.',
              "astra"
            ),
    ...
    {
      if (is_rtl()) {
        wp_enqueue_style("astra-notices-rtl", ASTRA_THEME_URI . "inc/assets/css/astra-notices-rtl.css", [], ASTRA_THEME_VERSION);
      } else {
        wp_enqueue_style("astra-notices", ASTRA_THEME_URI . "inc/assets/css/astra-notices.css", [], ASTRA_THEME_VERSION);
      }
    }
    ...
    public static function register_scripts($hook)
    {
       ...
       */
      $js_handle = apply_filters("astra_admin_script_handles", ["jquery", "wp-color-picker"]);
      ...
      if (!is_customize_preview()) {
        ...
        wp_register_script("astra-admin-settings", ASTRA_THEME_URI . "inc/assets/js/astra-admin-menu-settings.js", ["jquery", "wp-util", "updates"], ASTRA_THEME_VERSION, false);
    ...
    public static function styles_scripts()
    {
      // Styles.
      if (is_rtl()) {
        wp_enqueue_style("astra-admin-settings-rtl", ASTRA_THEME_URI . "inc/assets/css/astra-admin-menu-settings-rtl.css", [], ASTRA_THEME_VERSION);
      } else {
        wp_enqueue_style("astra-admin-settings", ASTRA_THEME_URI . "inc/assets/css/astra-admin-menu-settings.css", [], ASTRA_THEME_VERSION);
      }
    ...
    public static function menu_callback()
    {
      ...
      $ast_wrapper_class = apply_filters("astra_welcome_wrapper_class", [$current_slug]);
      ?>
					<div class="ast-menu-page-wrapper wrap ast-clear <?php echo esc_attr(implode(" ", $ast_wrapper_class)); ?>">
						...
						<!--								<a href="--><?php //echo esc_url( $ast_visit_site_url );
      ?><!--" target="_blank" rel="noopener" >-->
						<!--								--><?php //if ( $ast_icon ) {
      ?>
						<!--									<img src="--><?php //echo esc_url( ASTRA_THEME_URI . 'inc/assets/images/astra.svg' );
      ?><!--" class="ast-theme-icon" alt="--><?php //echo esc_attr( self::$page_title );
      ?><!-- " >-->
						<!--									<span class="astra-theme-version">--><?php //echo esc_html( ASTRA_THEME_VERSION );
      ?><!--</span>-->
						<!--								--><?php //}
      ?>
    ...
    public static function astra_welcome_page_starter_sites_section()
    {
        ...
        return;
					...
					<div class="postbox">
						...
						<div class="inside">
							<p>
				  <?php
      $astra_starter_sites_doc_link = apply_filters(
        "astra_starter_sites_documentation_link",
        astra_get_pro_url("https://wpastra.com/docs/installing-importing-astra-sites", "astra-dashboard", "how-astra-sites-works", "welcome-page")
      );
      ...
      printf(
        ...
        self::$page_title,
        !empty($astra_starter_sites_doc_link)
          ? "<a href=" . esc_url($astra_starter_sites_doc_link) . ' target="_blank" rel="noopener">' . esc_html($astra_starter_sites_doc_link_text) . "</a>"
          : esc_html($astra_starter_sites_doc_link_text)
      );
      ?>
							</p>
							<p>
				  <?php esc_html_e("Import your favorite site in one click and start your project with style!", "astra"); ?>
							</p>
				<?php
    $ast_sites_notice_btn = self::astra_sites_notice_button();
    ...
    printf(
      ...
      '%1$s',
      !empty($astra_knowledge_base_doc_link)
        ? "<a href=" . esc_url($astra_knowledge_base_doc_link) . ' target="_blank" rel="noopener">' . esc_html($astra_knowledge_base_doc_link_text) . "</a>"
    ...
    printf(
      /* translators: %1$s: Astra Knowledge doc link. */
      '%1$s',
      !empty($astra_community_group_link)
        ? "<a href=" . esc_url($astra_community_group_link) . ' target="_blank" rel="noopener">' . esc_html($astra_community_group_link_text) . "</a>"
    ...
    printf(
      /* translators: %1$s: Astra Knowledge doc link. */
      '%1$s',
      !empty($astra_support_link) ? "<a href=" . esc_url($astra_support_link) . ' target="_blank" rel="noopener">' . esc_html($astra_support_link_text) . "</a>" : esc_html($astra_support_link_text)
    );?>
						</div>
					</div>
			<?php
    ...
    public static function astra_welcome_page_content()
    {
      ...
      // Quick settings.
      $quick_settings = apply_filters("astra_quick_settings", [
        "logo-favicon" => [
      ...
      $extensions = apply_filters("astra_addon_list", [
      ...
      ?>
					<div class="postbox">
						<h2 class="hndle ast-normal-cusror"><span><?php esc_html_e("Settings:", "astra"); ?></span></h2>
						<div class="ast-quick-setting-section">
				<?php if (!empty($quick_settings)): ?>
									<div class="ast-quick-links">
										<ul class="ast-flex">
						<?php foreach ((array) $quick_settings as $key => $link) {
        echo '<li class=""><span class="dashicons ' .
          esc_attr($link["dashicon"]) .
          '"></span><a class="ast-quick-setting-title" href="' .
          esc_url($link["quick_url"]) .
          '" target="_blank" rel="noopener">' .
          esc_html($link["title"]) .
          "</a></li>";
      } ?>
			...
			<?php self::min_addon_version_message(); ?>
					...
					<!--			<div class="postbox">-->
					<!--				<h2 class="hndle ast-normal-cusror ast-addon-heading ast-flex"><span>--><?php //echo esc_html( $astra_addon_tagline );
      ?><!--</span>-->
      ...
      ?>
					...
					<!--									--><?php //									foreach ( (array) $extensions as $addon => $info ) {
    ...
    public static function astra_available_plugins()
    {
      ...
      $recommended_plugins = apply_filters("astra_recommended_plugins", [
        "astra-import-export" => [
      ...
      if (apply_filters("astra_show_free_extend_plugins", true)) { ?>
							...
							<!--				<div class="postbox">-->
							<!--					<h2 class="hndle ast-normal-cusror ast-addon-heading ast-flex"><span>--><?php
        //echo esc_html( $astra_addon_tagline );
        ?><!--</span>-->
							<!--					</h2>-->
							<!--						<div class="ast-addon-list-section">-->
							<!--							--><?php
    ...
    public static function top_header_right_section()
    {
      ...
      $top_links = apply_filters("astra_header_top_links", [
        "astra-theme-info" => [
          "title" =>
            '<img src=" ' .
            ASTRA_THEME_URI .
            'inc/assets/images/lightning-speed.svg" class="astra-lightning-icon" alt="Astra Lightning Speed">' .
            __(" Lightning Fast & Fully Customizable WordPress theme!", "astra"),
        ],
      ]);
      ...
      if (!empty($top_links)) { ?>
							<div class="ast-top-links">
								<ul>
					<?php foreach ((array) $top_links as $key => $info) {
